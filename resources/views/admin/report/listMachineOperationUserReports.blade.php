			...
			<div class="ragistration-detele-map">
				<div class="detele-tab">
					<ul class="nav nav-tabs">
						<li>							
							<span> {{!(empty($machine->machine))?ucfirst($machine->machine_id):''}} </span>					
						</li> 						    
					</ul>
				</div>
			</div>
				...
				<div class="tab-pane fade active in show" id="machine">
					<div class="ragistration-detele-section">
						<div class="machine-detele">
								@if($machineOperationUser->isNotEmpty())
							<table>
								<tr>
									<th> User Name </th>
								...
								<tr>
									...
									<td> 
										{{date('d-m-Y', strtotime($value->created_at)).' | '.date('H:s:i', strtotime($value->created_at))}}
									</td>
									
									<td>
										<a href="javascript:void(0);" onclick="getUserDirectiveAnsPreview(<?php echo $value->user_id; ?>,<?php echo $value->machine_id; ?>,<?php echo $machine->user_dir_id; ?>,<?php echo $value->id; ?>)"><img src="{{asset('public/images/eye-icon-d.png')}}"></a>
									</td>
								</tr>
								@endforeach								
							</table>
							...
							<h5 class="no-response">No record found.</h5>
							@endif
						</div>					
					</div>
